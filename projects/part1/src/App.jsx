const App = () => {
  const course = {
    name: 'Half Stack application development',
    parts: [
      {
        name: 'Fundamentals of React',
        exercises: 10
      },
      {
        name: 'Using props to pass data',
        exercises: 7
      },
      {
        name: 'State of a component',
        exercises: 14
      }
    ]
  }

  return (
    <div>
      <Header course={course.name}/>
      <Content parts={course.parts}/>
      <Total parts={course.parts}/>
    </div>
  )
}

const Header = (props) => { //Harus pakai props untuk passing data
  return (
    <>
      <h1>{props.course}</h1>
    </>
  )
}

const Content = (props) => { //Harus pakai props untuk passing data
  return (
    <>
      <Part name={props.parts[0].name} num={props.parts[0].exercises}/>
      <Part name={props.parts[1].name} num={props.parts[1].exercises}/>
      <Part name={props.parts[2].name} num={props.parts[2].exercises}/>
    </>
  )
}

const Part = (props) => { //Harus pakai props untuk passing data
  return (
    <>
      <p>
        {props.name} {props.num}
      </p>
    </>
  )
}

const Total = (props) => { //Harus pakai props untuk passing data
  return (
    <>
      <p>Number of exercises {props.parts[0].exercises + props.parts[1].exercises + props.parts[2].exercises}</p>
    </>
  )
}

export default App //Harus ada export default